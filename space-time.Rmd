...
Or get daily PRISM data with functions from the {prism} package for a single year.
```{r}
library(prism)

options(prism.path = "Data/PRISM/1992")
...
Create the folder and file names as character strings.
```{r}
library(lubridate)

dates <- seq(as_date("1990-01-01"), 
             as_date("1990-12-31"), 
             by = "day")
...
Read the rasters for one year and create a brick.
```{r}
library(here)
tmax.b <- NULL
for(i in 9:1){
...
tmax.b <- NULL
for(i in 19:20){
tmax.r <- raster(paste0("Data/PRISM/PRISM_tmin_stable_4kmD2_200806", i, "_bil/PRISM_tmin_stable_4kmD2_200806", i, "_bil.bil"))
tmax.b <- brick(tmax.r, tmax.b)
}
```
