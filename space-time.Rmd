...
Create a simple space-time data frame.
```{r}
days <- seq(as.Date('2019-01-01'), as.Date('2019-01-10'), by = 1)

df <- data.frame(x = rnorm(10), y = rnorm(10), t = days)

df_sf <- st_as_sf(df, coords = c("x", "y"))
df_st <- st_as_stars(df_sf)
```
