...
for(year in 1981:1990){
  ...
  mtrx <- st_intersects(Counties.sf, 
                        tracks, 
                        sparse = FALSE)
  counts.v <- c(rowSums(mtrx), counts.v)
}

years <- year(seq(as_date("1981-01-01"), 
                  as_date("1990-12-31"), 
                  by = "year"))
...
ggplot() +  
  geom_stars(data = counts.st, alpha = .8, downsample = c(1, 1, 1)) + 
  facet_wrap("year") +
  scale_fill_viridis_c() +
  coord_sf() +
  theme_map() +
